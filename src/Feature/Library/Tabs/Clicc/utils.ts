...
export const getCitationText = (
    ...
    const text = `${author ?? ""}, "${
        header ?? ""
    }", Mark Klamberg, Jonas Nilsson and Antonio Angotti (eds.), Commentary on the Law of the International Criminal Court - The Rome Statute, Lexsitus available at ${currUrl}, last accessed at ${getCurrentDate()}`;
    return text;
};
